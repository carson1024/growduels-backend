...
const rollCheckSendBetData = (data) => {
    if(data === undefined || data === null) {
        ...
        throw new Error(`You can only bet a min amount of DL${parseFloat(process.env.ROLL_MIN_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    }
}
...
const rollCheckSendBetUser = (data, user, userStats) => {
    if(data === undefined || user === undefined) {
        ...
        throw new Error(`You can only bet a total max amount of DL${parseFloat(process.env.ROLL_MAX_AMOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    } else if(Math.floor(data.amount * (data.multiplier / 100) + userStats.winnings) > Math.floor(process.env.ROLL_MAX_PROFIT * 1000)) {
        throw new Error(`You can only have a total max win amount of DL${parseFloat(process.env.ROLL_MAX_PROFIT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} per game.`);
    }
}
...
const rollGenerateGame = () => {
    return new Promise(async(resolve, reject) => {
		...
		// Create new server seed for new game
                RollSeed.create({
                    seedPublic: seedClient,
